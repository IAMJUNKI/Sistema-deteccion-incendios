...
def _longitud_vias_por_celda(grid: gpd.GeoDataFrame, roads: gpd.GeoDataFrame) -> pd.Series:
    ...
    for start in range(0, len(grid), 500):
        ...
        grid_positions, road_positions = roads.sindex.query(
            subset.geometry, predicate="intersects"
        )
...
def _fraccion_poligonos_por_celda(grid: gpd.GeoDataFrame, polygons: gpd.GeoDataFrame) -> pd.Series:
    ...
    for start in range(0, len(grid), 500):
        subset = grid.iloc[start : start + 500]
        positions_grid, positions_polygon = polygons.sindex.query(
            subset.geometry, predicate="intersects"
        )
...
def calcular_variables_actividad_humana(
    ...
    buildings = (
        buildings if buildings is not None else gpd.GeoDataFrame(geometry=[], crs=active_grid.crs)
    ).to_crs(active_grid.crs)
    ...
    for variable, classes in ROAD_CLASS_GROUPS.items():
        result = result.merge(
            _longitud_vias_por_celda(active_grid, roads.loc[roads["fclass"].isin(classes)]).rename(
                variable
            ),
            on="cell_id",
            how="left",
    ...
    for variable, polygons in {
        ...
        result = result.merge(
            _fraccion_poligonos_por_celda(active_grid, polygons).rename(variable),
            on="cell_id",
            how="left",
        )
        result[variable] = result[variable].fillna(0.0)
    ...
    metadata = {
        ...
        **{
            name: {
                ...
                "units": "km",
                "description": "Road length inside the 1 km cell, classified from OSM fclass.",
            }
            for name in [*ROAD_CLASS_GROUPS, "road_length_other_km"]
        },
        "residential_area_fraction": {
            "long_name": "Residential area fraction",
            "units": "fraction",
            "description": "Fraction of cell covered by OSM residential land-use polygons.",
        },
        "building_area_fraction": {
            "long_name": "Building footprint fraction",
            "units": "fraction",
            "description": "Fraction of cell covered by OSM building footprints.",
        },
    }
    for variable in selected:
