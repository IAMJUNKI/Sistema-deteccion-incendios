...
> 1. **Artefactos Estáticos Inmutables (~11 MB):** Agrupan los modelos serializados calibrados por horizonte temporal (`forecast_risk_egif_48_t1/t2/t3.joblib`), sus contratos de metadatos (`.json`), el manifiesto de trazabilidad activa (`active_model_manifest.json`), la rejilla canónica de 29.601 celdas (`galicia_grid_1km_egif.parquet`) y las capas vectoriales cartográficas integradas directamente en el código de la aplicación (`src/webapp/assets/galicia_*.geojson` para el contorno de Galicia, límites provinciales y sectores orgánicos del PLADIGA, inmunes a desacoplamientos de volumen en producción).
